...
class OpenVeoClient {
  ...
  constructor(webServiceUrl, clientId, clientSecret, certificate) {
    ...
    if (!clientId || typeof clientId !== 'string')
      throw new TypeError(`Invalid web service client id : ${clientId}`);

    if (!clientSecret || typeof clientSecret !== 'string')
      throw new TypeError(`Invalid web service client secret : ${clientSecret}`);
   ...
   * @throws {TypeError} Thrown if method or endPoint is not valid a String
   */
    ...
    return new Promise((resolve, reject) => {
      ...
      const authenticateAndExecute = () => {
        ...
        return this.authenticate().then(() => {
          ...
          // Execute request
          return this.execute(options, body);
        });
      ...
      const getErrorMessage = (result) => {
        if (result.error) {
          if (result.httpCode === 403)
            return `You don't have the authorization to access the endpoint "${method} ${options.path}"`;
      ...
      authenticateAndExecute().then((result) => {
        if (result.error) {
          if (result.error_description && (result.error_description === 'Token not found or expired' ||
            ...
            authenticateAndExecute().then((result) => {
              if (result.error)
                reject(new RequestError(getErrorMessage(result), result.httpCode));
              else
                resolve(result);
            }).catch((error) => {
              reject(error);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.isAuthenticated())
        ...
        this.execute({
          ...
          if (result.error)
            return reject(new AuthenticationError(result.error_description));
